...
export async function getResumoMes(mes?: number, ano?: number): Promise<{ receitas: number; despesas: number; saldo: number }> {
...
export async function getUltimosLancamentos(dias: number = 7): Promise<any[]> {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    lancamentos.push({
      data: dataStr, tipo: r[2] || '', obra: r[3] || '',
      clienteFornecedor: r[4] || '', valor: r[5] || '', caixa: r[6] || '', formaPagamento: r[7] || '',
    })
  }
...
export async function getResumoObra(): Promise<any[]> {
  ...
  return Object.entries(obras).map(([obra, vals]) => ({ obra, receitas: vals.receitas, despesas: vals.despesas, saldo: vals.receitas - vals.despesas })).sort((a, b) => b.saldo - a.saldo)
}
...
export async function getApontamentos(tipo: 'dia' | 'mes' | 'data', data?: string): Promise<any[]> {
  ...
  if (tipo === 'data' && data) dataFiltro = parseDataBrasil(data)
  for (let i = 1; i < rows.length; i++) {
    const r = rows[i]
    if (!r || !r[0]) continue
    ...
    apontamentos.push({ data: r[0], funcionario: r[2] || '', tipo: r[3] || '', obra: r[4] || '', almoco: r[5] || '', veiculo: r[6] || '', km: r[7] || '', obs: r[8] || '' })
  }
...
export async function getUltimosLancamentosDashboard(dias: number = 7): Promise<any[]> {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const debito = parseValorMonetario(debStr)
    const credito = parseValorMonetario(credStr)
    lancamentos.push({ data: r[0], tipo: r[4] || '', obra: r[3] || '', clienteFornecedor: r[7] || '', valor: credito > 0 ? credito : debito, caixa: r[14] || '', formaPagamento: r[10] || '' })
  }
  lancamentos.sort((a, b) => {
    const da = parseDataBrasil(a.data)?.getTime() || 0
    const db = parseDataBrasil(b.data)?.getTime() || 0
...
export async function getFuncionarios(): Promise<Funcionario[]> {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (!r || !r[0]) continue
    funcionarios.push({
      codigo: (r[0] || '').trim(), apelido: (r[1] || '').trim(),
      nome: (r[2] || '').trim(), funcao: (r[3] || '').trim(),
    })
  }
  return funcionarios
}

type ListaAuxiliar = { tipo: string; valor: string }
...
export async function getListasAuxiliares(): Promise<ListaAuxiliar[]> {
  ...
  for (let i = 1; i < rows.length; i++) {
    const r = rows[i]
    if (!r[0] && !r[1]) continue
    listas.push({ tipo: (r[0] || '').trim(), valor: (r[1] || '').trim() })
  }
  return listas.filter(l => l.tipo || l.valor)
}

export async function addDespesa(d: { obra: string; fornecedor: string; categoria: string; subcategoria: string; descricao: string; valor: number; caixaOrigem: string; formaPagamento: string }) {
  const hojeStr = hoje()
  const getRes = await sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: "'Despesas'!A:A" })
  const linhas = getRes.data.values || []
  const proximaLinha = linhas.length + 1
  const resIds = await sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: "'Despesas'!B:B" })
  const idsExistentes = resIds.data.values || []
  let ultimoNumero = 0
  for (const row of idsExistentes) {
    const id = (row[0] || '').toString().trim()
    if (id.startsWith('DES-')) { const num = parseInt(id.replace('DES-', ''), 10); if (!isNaN(num) && num > ultimoNumero) ultimoNumero = num }
  }
  const novoId = `DES-${String(ultimoNumero + 1).padStart(3, '0')}`
  await sheets.spreadsheets.values.batchUpdate({
    spreadsheetId: SPREADSHEET_ID, requestBody: { valueInputOption: 'USER_ENTERED', data: [
      { range: `'Despesas'!A${proximaLinha}:B${proximaLinha}`, values: [[hojeStr, novoId]] },
      { range: `'Despesas'!C${proximaLinha}:J${proximaLinha}`, values: [[d.obra, d.fornecedor, d.categoria, d.subcategoria, d.descricao, d.valor, d.caixaOrigem, d.formaPagamento]] },
    ]},
  })
  return { id: novoId }
}

export async function addRecebimento(r: { obra: string; valor: number; parcela: string; formaPagamento: string }) {
  const hojeStr = hoje()
  const getRes = await sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: "'Recebimentos'!A:A" })
  const linhas = getRes.data.values || []
  const proximaLinha = linhas.length + 1
  const resIds = await sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: "'Recebimentos'!B:B" })
  const idsExistentes = resIds.data.values || []
  let ultimoNumero = 0
  for (const row of idsExistentes) {
    const id = (row[0] || '').toString().trim()
    if (id.startsWith('REC-')) { const num = parseInt(id.replace('REC-', ''), 10); if (!isNaN(num) && num > ultimoNumero) ultimoNumero = num }
  }
...
export async function addTransferencia(t: { caixaOrigem: string; caixaDestino: string; valor: number; subcategoria: string; descricao: string }) {
  const hojeStr = hoje()
  const getRes = await sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: "'Transferências'!A:A" })
  const linhas = getRes.data.values || []
  const proximaLinha = linhas.length + 1
  const resIds = await sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: "'Transferências'!A:A" })
  const idsExistentes = resIds.data.values || []
  let ultimoNumero = 0
  for (const row of idsExistentes) {
    const id = (row[0] || '').toString().trim()
    if (id.startsWith('TRA-')) { const num = parseInt(id.replace('TRA-', ''), 10); if (!isNaN(num) && num > ultimoNumero) ultimoNumero = num }
  }
  ...
  await sheets.spreadsheets.values.batchUpdate({
    spreadsheetId: SPREADSHEET_ID, requestBody: { valueInputOption: 'USER_ENTERED', data: [
      ...
      { range: `'Transferências'!C${proximaLinha}:H${proximaLinha}`, values: [[t.valor, t.caixaOrigem, t.caixaDestino, t.subcategoria, 'Pix', t.descricao]] },
    ]},
  })
  return 'ok'
}

// ✅ addApontamento com KM na coluna H
export async function addApontamento(a: { funcionario: string; tipo: string; obra: string; almoco: string; veiculo: string; obs: string; km?: number }) {
  const hojeStr = hoje()
  const getRes = await sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: "'Apontamento Diário'!A:A" })
  const linhas = getRes.data.values || []
  const proximaLinha = linhas.length + 1
  const resIds = await sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: "'Apontamento Diário'!B:B" })
  const idsExistentes = resIds.data.values || []
  let ultimoNumero = 0
  for (const row of idsExistentes) {
    const id = (row[0] || '').toString().trim()
    if (id.startsWith('APO-')) { const num = parseInt(id.replace('APO-', ''), 10); if (!isNaN(num) && num > ultimoNumero) ultimoNumero = num }
  }
  ...
  await sheets.spreadsheets.values.batchUpdate({
    spreadsheetId: SPREADSHEET_ID, requestBody: { valueInputOption: 'USER_ENTERED', data: [
      ...
      { range: `'Apontamento Diário'!C${proximaLinha}:I${proximaLinha}`, values: [[a.funcionario, a.tipo, a.obra, a.almoco, a.veiculo, a.km != null ? a.km.toString().replace('.', ',') : '', a.obs]] },
    ]},
  })
  return 'ok'
}

// ✅ addApontamentosBatch com KM na coluna H (LOTE)
export async function addApontamentosBatch(params: { data: string; obra: string; funcionarios: Array<{ funcionario: string; tipo: string; almoco: string; veiculo: string; obs: string; km?: number }> }) {
  ...
  for (let i = 0; i < qtd; i++) {
    ...
    ids.push(novoId)
    dataValues.push([data, novoId, f.funcionario, f.tipo, obra, f.almoco, f.veiculo, f.km != null ? f.km.toString().replace('.', ',') : '', f.obs])
  }
