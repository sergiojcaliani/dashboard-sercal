...
export default function DashboardPage() {
  ...
  async function carregarResumoMes(mes: number, ano: number) {
    try {
      ...
      if (res.ok) {
        ...
        setResumoMes(json?.data || null)
      }
    } catch (err) {
      console.error('Erro ao carregar resumo do mês:', err)
    }
  }
  ...
  useEffect(() => {
    async function carregar() {
      ...
      try {
        ...
        if (resCaixas.ok) {
          const json = await resCaixas.json()
          setCaixas(json?.data || [])
        }
        if (resLanc.ok) {
          const json = await resLanc.json()
  ...
  useEffect(() => {
    carregarResumoMes(mesSelecionado, anoSelecionado)
  ...
  async function carregarObras() {
    try {
      ...
      if (res.ok) {
        const json = await res.json()
        setResumoObra(json?.data || [])
      }
    } catch (err) {
      console.error('Erro ao carregar resumo por obra:', err)
    }
  }
  ...
  return (
    <div className="max-w-6xl mx-auto px-3 sm:px-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6">
        <button onClick={() => {
          setMostrarObras(!mostrarObras)
          if (!mostrarObras) carregarObras()
        }}
        ...
        <button onClick={() => {
          setMostrarApontamentos(!mostrarApontamentos)
          if (!mostrarApontamentos) {
            setTipoApontamento('dia')
            carregarApontamentos('dia')
          }
        }}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2 sm:mb-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <button onClick={() => { setTipoApontamento('dia'); setDataApontamento(''); carregarApontamentos('dia') }}
                className={`px-3 sm:px-4 py-1.5 rounded-lg text-xs sm:text-sm font-medium transition ${tipoApontamento === 'dia' ? 'bg-purple-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                Hoje
              </button>
              <button onClick={() => { setTipoApontamento('mes'); setDataApontamento(''); carregarApontamentos('mes') }}
                className={`px-3 sm:px-4 py-1.5 rounded-lg text-xs sm:text-sm font-medium transition ${tipoApontamento === 'mes' ? 'bg-purple-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                Mês
              </button>
              <input
                type="date"
                ...
                onChange={e => {
                  ...
                  if (val) {
                    ...
                    setDataApontamento(val)
                    setTipoApontamento('data')
                    carregarApontamentos('data', dataBR)
                  }
                }}
